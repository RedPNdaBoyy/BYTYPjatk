...
public class Money implements Comparable {
	...
	double pom =Math.round((double) amount)/100.0;
	String odp= (((pom==0) ? "0,00" : df.format(pom))).replace(".",",");
	return odp+" "+currency.getName();
	}
	...
	int sum =  amount+currency.valueInThisCurrency(other.amount, other.currency);
	...
	int diff =  amount - currency.valueInThisCurrency(other.amount,other.currency);
	...
	public int compareTo(Object other)
		...
		if (this == other)
			{
			return 0; // Same object, so equal
			}

		if (!(other instanceof Money))
			{
			throw new ClassCastException("Object is not an instance of Money");
			}

		Money Oother = (Money) other;
		int thisUniversalValue = this.universalValue();
		int otherUniversalValue = Oother.universalValue();

		if (thisUniversalValue < otherUniversalValue)
			{
			return -1;
			} else if (thisUniversalValue > otherUniversalValue)
			{
			return 1;
			} else
			{
